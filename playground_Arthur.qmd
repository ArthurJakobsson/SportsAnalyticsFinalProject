...
# init_qpoiss <- glm.nb(total_yards_gained ~ yardline_100+runPercent+passPercent+as.factor(posteam_type), data = dbd_no_neg)
# summary(init_qpoiss)
dbd_no_neg$drivePoints[dbd_no_neg$drivePoints<0] <- 0 

ZIP_model <- zeroinfl(drivePoints ~yardline_100+runPercent+passPercent+as.factor(posteam_type), data=dbd_no_neg, dist = "poisson", link="logit")
summary(ZIP_model)
```
